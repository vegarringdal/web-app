...
CREATE OR REPLACE FUNCTION F_GET_CURRENT_USER
    RETURN VARCHAR IS
BEGIN
    -- IF WEB CLIENT ID WILL HAVE USER
...
CREATE TRIGGER TR_WEB_USER_AUDIT
BEFORE
    INSERT OR UPDATE
    ON T_WEB_USER
    FOR EACH ROW
...
CREATE TRIGGER TR_WEB_ROLE_AUDIT
BEFORE
    INSERT OR UPDATE
    ON T_WEB_ROLE
    FOR EACH ROW
...
CREATE TRIGGER TR_WEB_USER_ROLE_AUDIT
BEFORE
    INSERT OR UPDATE
    ON T_WEB_USER_ROLE
    FOR EACH ROW
...
CREATE TRIGGER TR_WEB_REST_API_AUDIT
BEFORE
    INSERT OR UPDATE
    ON T_WEB_REST_API
    FOR EACH ROW
...
CREATE TRIGGER TR_WEB_GRID_CONFIG_AUDIT
BEFORE
    INSERT OR UPDATE
    ON T_WEB_GRID_CONFIG
    FOR EACH ROW
    BEGIN
        IF INSERTING THEN
            :NEW.CREATED_BY := F_GET_CURRENT_USER();
...
CREATE VIEW AI_WEB_USER_ROLE AS
 SELECT 
    ...
    FROM 
        T_WEB_USER_ROLE A
    LEFT JOIN 
        T_WEB_ROLE B ON B.ID = A.WEB_ROLE_ID
    LEFT JOIN 
        T_WEB_USER C ON C.ID = A.WEB_USER_ID;
